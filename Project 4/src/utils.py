...
def demo(
        agent: Agent,
        environment_name: str=ENVIRONMENT_NAME,
        directory: str='./results/demos'               # directory to save videos
    ):
    ...
    # Set agent to eval mode if it's a PyTorch module
    if hasattr(agent, 'eval'):
        agent.eval()
    if hasattr(agent, 'to'):
        agent.to(demo_device)
    ...
    # apply video recording wrapper - record only the first episode
    demo_environment = RecordVideo(
        demo_environment,
        video_folder=directory,
        episode_trigger=lambda x: x == 0,  # Only record episode 0 (first episode)
        name_prefix=f'{world_name}-{race_type}-{agent.name}'
    )

    # Run a single episode for the video (~30 seconds for a complete lap)
    ...
    for t in range(MAX_DEMO_STEPS):
        ...
        # Convert observation to numpy array (agent.get_action expects numpy)
        obs_numpy = np.asarray(observation, dtype=np.float32)
        action = agent.get_action(obs_numpy, deterministic=True)
        
        # Ensure action is a scalar integer for discrete action space
        ...
        observation, reward, terminated, truncated, info = demo_environment.step(
            action
        )
        ...
        if terminated or truncated:
            # Log episode completion - extract progress from info dict
            progress = 0
            if isinstance(info, dict):
                # Try different possible keys for progress
                if 'progress' in info:
                    progress = info['progress']
                elif 'reward_params' in info and isinstance(info['reward_params'], dict):
                    progress = info['reward_params'].get('progress', 0)
                elif hasattr(demo_environment, 'unwrapped'):
                    # Try to get progress from unwrapped environment
                    unwrapped = demo_environment.unwrapped
                    if hasattr(unwrapped, 'progress'):
                        progress = unwrapped.progress
            
            logger.info(f"Demo episode completed: Reward={episode_reward:.2f}, Steps={episode_steps}, Progress={progress:.1f}%")
            break
...
def evaluate_track(
        ...
        world_name: str,
        environment_name: str=ENVIRONMENT_NAME,
        directory: str='./results/evaluations'               # directory to save eval data
    ):
    race_type = get_race_type(
        environment_params_path=ENVIRONMENT_PARAMS_PATH
    )
...
def plot_metrics(
        data,
        title,
        directory: str='./results/plots'               # directory to save plots
    ):
